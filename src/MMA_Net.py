...
class Up(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            ...
            self.conv = DoubleConv(in_channels, in_channels // 4)
        else:
...
class Up2(nn.Module):
    def __init__(self, in_channels, out_channels, bilinear=True):
        ...
        if bilinear:
            self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)
            self.conv = DoubleConv(in_channels, in_channels // 5)
        else:
...
class SAMConv(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv2 = DoubleConv(32, out_channels)
...
class MGNet(nn.Module):
    def __init__(self, in_channels,  seg_prior, boundary_prior, n_class):
        ...
        # 初始训练为16
        self.MM1 = RFB_modified(64, 2)
        self.MM2 = RFB_modified(128, 2)
        self.MM3 = RFB_modified(256, 2)
        self.MM4 = RFB_modified(512, 2)
        self.MM = MMA(8, 128)
        self.Rs = RS(640, 512)
        ...
        self.ap = OutConv(3, 2)
        self.cp = SAMConv(1, 2)
        self.SAM = OutConv(1156, 256)

        # val
        # self.smx = OutConv(1152, 1024)

        # 固定
        self.sam4 = OutConv(256, 256)
        self.sam3 = OutConv(256, 128)
        self.sam2 = OutConv(256, 64)
        self.sam1 = OutConv(256, 32)
        ...
        self.sg2 = SG2(256, 64)
        self.sg1 = SG1(128, 32)
        self.decoder5 = DoubleConv(256, 512)
    ...
    def forward(self, x, y, z):
        ...
        area = F.interpolate(y, scale_factor=1/16, mode='bilinear')
        area = self.ap(area)
        # area = area.expand(-1, 2, -1, -1)
        coutour = F.interpolate(z, scale_factor=1 / 16, mode='bilinear')
        # coutour = coutour.expand(-1, 2, -1, -1)
        coutour = self.cp(coutour)
        afrs = torch.cat((rs, area), dim=1)
        cmma = torch.cat((afrs, mma), dim=1)
        samcpx = torch.cat((cmma, coutour), dim=1)
        osam = torch.cat((samcpx, x), dim=1)
        sam = self.SAM(osam)

        # else:
        # sam = torch.cat((rs, x, mma), dim=1)
        # sam = self.smx(sam)
...
if __name__ == '__main__':
    ...
    # model = MGNet(in_channels=3, n_class=2).cuda()
    ...
    flops, params = profile(model, inputs=(input_tensor, seg_tensor, bou_tensor), verbose=False)
